...
<xsl:stylesheet xmlns="urn:isbn:1-931666-22-9" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- This stylesheet looks at ancestors containing dated descendants (@normal only).
    It ignores the old unitdate, if there was one, and creates a unitdate with @normal with the earliest and latest dates. 
    ...
    <xsl:template match="@*|node()|comment()|processing-instruction()">
        <xsl:copy>
            <xsl:apply-templates select="@*[not(name()='calendar') and not(name()='era')]|node()|comment()|processing-instruction()"/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template
        match="ead:c[descendant::ead:c/ead:did/ead:unitdate[@normal]]">
...
<xsl:copy>
    ...
    <did>       
        ...
        <xsl:apply-templates select="ead:did/*[not(self::ead:unittitle or self::ead:unitdate)]"/>
</did>
