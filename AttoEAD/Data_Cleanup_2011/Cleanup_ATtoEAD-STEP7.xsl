...
<xsl:stylesheet xmlns="urn:isbn:1-931666-22-9" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="ead:dao">
        <xsl:copy>
            <xsl:copy-of select="@*[not(name()='ns2:actuate') and not(name()='ns2:show') and not(name()='ns2:role') and not(name()='ns2:href')]"/>
            <xsl:attribute name="xlink:href"><xsl:value-of select="@ns2:href"></xsl:value-of></xsl:attribute>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
