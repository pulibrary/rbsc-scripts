...
<xsl:stylesheet version="2.0" xmlns:ead="urn:isbn:1-931666-22-9"
    ...
    <xsl:template mode="single"
        ...
        <xsl:choose>
        <xsl:when test="../../self::*[@level='item' and (following-sibling::ead:c[@level='item'] or preceding-sibling::ead:c[@level='item'])]">
            <xsl:if test="../../self::*[@level='item' and not(preceding-sibling::ead:c[@level='item'])]">
                ...
                <fo:inline>
                    ...
                    <xsl:value-of select="../../../ead:did/ead:unitdate[not(position()=last())]"/> /                                 
                    <xsl:value-of select="../../../ead:did/ead:unitdate[position()=last()]"/> (Various Items)
                </fo:inline>   
                            </fo:block>
                        </fo:block>
                    </fo:table-cell>
